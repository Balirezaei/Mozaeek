...
namespace MozaeekCore.ApplicationService.Command
{
    public class CreateRequestOrgCommandHandler : IBaseAsyncCommandHandler<CreateRequestOrgCommand, CreateRequestOrgCommandResult>
    {
        ...
        {
            var requestOrg = new RequestOrg(0, cmd.Title,cmd.Icon, cmd.ParentId);
            await _repository.Add(requestOrg);

            await _unitOfWork.CommitAsync();
            await _publisher.PublishAsync(new RequestOrgCretedOrUpdated(requestOrg.Id, requestOrg.Title,requestOrg.Icon, requestOrg.ParentId, true));
