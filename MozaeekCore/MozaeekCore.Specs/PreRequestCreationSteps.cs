...
{
    [Binding]
    public class PreRequestCreationSteps
    {
        private PreRequest request;
        ...
        {
            request = table.CreateInstance<PreRequest>();
        }
        ...
        public void ThenTheResultShouldBeAnUnProcessedRequestWithIsProcessedFalseValue()
        {
            ...
            using (var httpClientHandler = new HttpClientHandler())
            {
                ...
                using (var client = new HttpClient(httpClientHandler))
                {
                    ...
                    var result = JsonConvert.DeserializeObject<PreRequestResult>(responseBody);
