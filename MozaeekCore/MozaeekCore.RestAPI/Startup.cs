...
namespace MozaeekCore.RestAPI
{
    public class Startup
    {
        ...
        public void ConfigureServices(IServiceCollection services)
        {
            ...
            services.AddScoped<IErrorHandling, SerilogErrorHandle>(sp =>
            {
                var path = Configuration.GetValue<String>("SailorLogPath");
                
                Log.Logger = (Serilog.ILogger)new LoggerConfiguration()
                    .WriteTo
                    .File($"{path}log-.txt", LogEventLevel.Verbose, "{Timestamp:yyyy-MM-dd HH:mm:ss.fff zzz} [{Level:u3}] {Message:lj}{NewLine}{Exception}", (IFormatProvider)null, new long?(1073741824L), (LoggingLevelSwitch)null, false, false, new TimeSpan?(), RollingInterval.Day, false, new int?(10000), (Encoding)null)
                    .MinimumLevel.Information().CreateLogger();
                return new SerilogErrorHandle();
            });
            ...
            services.Configure<RestApiAppSettings>(Configuration);
        }
