...
export const getStorageExpiration = (key: string, relative: boolean = false) => {
...
/*  setStorage: writes a key into localStorage setting a expire time
    ...
    returns:
        <boolean> : telling if operation succeeded
 */
export function setStorage(key: string, value: string, expiresInSeconds: number = LocalStorageExpiration.Infinite) {
  const now = Date.now(); //milliseconds since epoch time, lets deal only with integer
  const schedule = now + expiresInSeconds * 1000;
  try {
