...
const useHttpCall = <Fn extends (data: any) => Promise<ApiResponse<any> | { ___customCancel___: boolean }>>(
  httpPromise: Fn | undefined,
  ...
  const call = async <T>(data: Parameters<Fn>[0], throwOnError?: boolean, delayMilliseconds?: number) => {
    if (!httpPromise) {
      throw Error('httpPromise is undefined.');
    }
  ...
  const callFullResponse = async <T>(data: Parameters<Fn>[0], throwOnError?: boolean, delayMilliseconds?: number) => {
    if (!httpPromise) {
      throw Error('httpPromise is undefined.');
    }
  ...
  const cancel = () => {
    if (options && options.apiDataItem) {
      cancelRequest(options.apiDataItem, options.cancelMessage ?? 'Request cancelled');
    }
    reset();
  };
  ...
  return { call, callFullResponse, error: state.error, pending: state.pending, success: state.success, response: state.response, reset, cancel, mock };
};
