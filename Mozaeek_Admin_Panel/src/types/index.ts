...
export interface AxiosCustomRequestConfig extends AxiosAuthRefreshRequestConfig {
  apiDataItem?: ApiDataItem;
  disableRedirectOnUnauthorized?: boolean;
}
...
export type AllStringPartial<T> = Partial<AllString<T>>;

export type SagaAppApiResponse<T> = {
  data: T;
  status: number;
};
