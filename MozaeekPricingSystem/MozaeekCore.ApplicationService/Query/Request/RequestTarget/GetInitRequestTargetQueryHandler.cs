...
namespace MozaeekCore.ApplicationService.Query
{
    public class GetInitRequestTargetQueryHandler : IBaseAsyncQueryHandler<FindByKeyEditMode, InitRequestTargetDto>
    {
        ...
        {
            var labels = await _labelQueryService.GetByPredicate(m => m.Id > 0);
            var subjects = await _subjectQueryService.GetByPredicate(m => m.Id > 0);
            var requestOrgs = await _requestOrgQueryService.GetByPredicate(m => m.HasChild == false);
